...
const Contact: NextPage = () => {
  ...
  const sendEmail = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    if (form.email === "" || form.subject === "" || form.msg === "")
      setMsgAlert("Veuillez remplir tous les champs");
    else {
      if (!validEmail(form.email))
        setMsgAlert("Veuillez vérifier le format de l'email");
      else {
        ...
        <div className="mt-16 w-11/12 flex flex-col md:flex-row md:w-8/12 md:space-x-10 items-center">
          <form className="w-full flex flex-col">
            ...
            <input
              ...
              id="email"
            />
            <label htmlFor="subject" className="mt-4 font-mono uppercase">
              Sujet
            </label>
            <input
              value={form.subject}
              ...
              id="subject"
            />
            <label htmlFor="msg" className="mt-4 font-mono uppercase">
              Message
            </label>
            <textarea
              value={form.msg}
              onChange={(e) => handleInputs(e)}
              id="msg"
              className="transition-all duration-200 p-2 resize-none h-32 outline-none text-slate-900 focus:bg-zinc-100"
            ></textarea>
            <button
